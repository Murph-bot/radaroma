    ...
    <div className="space-y-8">
            ...
            return (
              <label
                ...
                className={`flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${
                  checked ? "bg-amber-100 text-amber-900" : "hover:bg-stone-100"
      ...
      {chosen.length < 2 ? (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div className="flex items-center justify-center rounded-xl border border-stone-200 bg-stone-50 p-6">
            <RadarChart series={chartSeries} size={300} className="text-stone-800" />
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-200 text-left">
                  <th className="py-2 pr-4 font-medium text-stone-500">Axis</th>
                  {chosen.map((r) => (
                    <th key={r.cafe.id} className="py-2 pr-4 font-semibold text-stone-800">
                      <Link href={`/cafes/${r.cafe.slug}`} className="hover:text-amber-800">
                        {r.cafe.name}
                      </Link>
                    </th>
                  ))}
              ...
              <tbody>
                {SCORE_AXES.map((axis: ScoreAxis) => (
                  <tr key={axis} className="border-b border-stone-100">
                    <td className="py-2 pr-4 text-stone-600">{AXIS_LABELS[axis]}</td>
                    {chosen.map((r) => (
                      <td key={r.cafe.id} className="py-2 pr-4 font-mono text-stone-800">
                ...
                <tr>
                  <td className="py-2 pr-4 font-medium text-stone-600">Overall</td>
                  {chosen.map((r) => (
                    <td key={r.cafe.id} className="py-2 pr-4 font-mono font-semibold text-amber-900">
