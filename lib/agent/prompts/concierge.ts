...
export function conciergeSystemPrompt(cafeContext?: string, locale?: "en" | "el"): string {
  ...
  const language =
    locale === "el"
      ? "\n- The site is being served in Greek — reply in Greek (keep café names and neighborhoods as written in the dataset)."
      : ""
  return (
...
- Be warm, concise, and concrete: name 2-4 cafés with a one-line reason each. 2-4 sentences is the sweet spot.${language}`
  )
}
